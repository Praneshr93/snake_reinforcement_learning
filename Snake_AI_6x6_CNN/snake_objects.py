    ...
    def next_move(self, direction):
        ...
        if (
            ...
            if new_X == self.fx and new_Y == self.fy:
                ...
                # Clear the tail
                # reward for moving closer/further to food
                self.reward = (
                    (abs(self.fx-self.X[1]) - abs(self.fx-self.X[0]))
                    + (abs(self.fy-self.Y[1]) - abs(self.fy-self.Y[0]))
                    ) * rewards["closer"]
                self.X.pop()
                self.Y.pop()
                self.XY.pop()
                ...
                if (
                    ...
                    or (next_XY in self.XY[:-1])
                ):
                    self.reward = self.reward + rewards["danger"]
            self.board_values = self.get_board_vals()
        else:
