...
E_DEL = 0.0002        # ε-reduction rate for the ε-greedy policy
E_MIN = 0.0001           # minimum ε value for the ε-greedy policy
...
num_games = 10000       # number of games to run
...
q_network = tf.keras.Sequential([
    tf.keras.layers.Input(shape=(board_width, board_width, 3)),
    tf.keras.layers.Conv2D(16, 3, activation='swish'),
    tf.keras.layers.Conv2D(32, 5, activation='swish'),
...
for game in range(num_games):
    ...
    while not new_board.game_over:
        ...
        state_qn = np.reshape(board_values, (1, board_width, board_width, 3))
    ...
    # Update the ε value
    epsilon = max(E_MIN, epsilon - E_DEL)
